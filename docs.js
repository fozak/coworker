/*
all data and schemas are from Frappe stored in 1 collection item in PocketBase
...
Every record in record.data is exactly copying json schema structure  of Frappe and related documents. there is also record.meta and record.users that I can use to set up Permissions
The document status follows the Frappe "docstatus": 0,1,2 

As I work in pocketbase I want to have
- list and view - I want users from my funcitonal group to be able to see their own data, also group-related data 
- update - I want only owner to update (if "docstatus": 0 still, is owner)
- delete - No deletion - we set "docstatus": 2 */
...
{    
    ...
    "facts": {
        "why_need": default PocketBase we can not without it,
    }
    "collectionName": "item",
    "facts": {
        "why_need": Main storage all Frappe like doctypes and Frappe like schemas stored in this
    }
    "issues": {
        "auth rules": "Potential issues with data integrity and access control.",
        "how_to_reproduce": "Attempt to access user data without proper permissions."
    },
    "collectionName": "userView",
    "facts": {
        "why_need": Auth rules for item collection need to check some data it can not,
        "how data is produced": ./js/pb-functions.js?@@func createViewuserView
    }
}



user = await pb.getDoc('mbl.acc4@gmail.com');
user:
{
    "collectionId": "pbc_940982958",
    "collectionName": "item",
    "created": "2025-07-18 01:38:18.792Z",
    "data": {
        "_assign": null,
        "_comments": null,
        "_liked_by": null,
    ...
    "name": "mbl.acc4@gmail.com",
    "updated": "2025-07-18 01:38:18.792Z"
}

Every doctyope has the its correspondend schema in the same collection
schema = await pb.getSchema('User'):
{
    "actions": [],
    "allow_import": 1,
    "allow_rename": 1,
...
there are all correspondent Roles and Permissions defined in the system with identical to Frappe docytpes:
doctypes: Has Role, Role...
